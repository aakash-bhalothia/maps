...
def find_centroid(cluster):
    ...
    longs = []
    for r in cluster:
        lats += [restaurant_location(r)[0]]
        longs += [restaurant_location(r)[1]]
...
def best_predictor(user, restaurants, feature_fns):
    ...
    # BEGIN Question 8
    featurelist = [x for x in feature_fns]
    list1=[find_predictor(user, reviewed, feature_fn) for feature_fn in featurelist]
    predictorlist = [x for (x,y) in list1]
    rsquaredlist=[y for (x,y) in list1]
    rvalue=rsquaredlist.index(max(rsquaredlist))
    return predictorlist[rvalue]
...
def rate_all(user, restaurants, feature_fns):
    ...
    for r in restaurants:
        if r in reviewed:
            predlist += [(restaurant_name(r),user_rating(user, restaurant_name(r)))]
        else:
            predlist += [(restaurant_name(r),predictor(r))]
    return dict(predlist)
